...
def draw_grid(surface, dimy, dimx):
    ...
    for i in range(dimx):
        ...
        # long lengths to assure they reach from one side of the screen to the other
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RIGHT:
                        counter += 1
                        pygame.time.delay(5) # Protect inputs to get more descrete movement
...
def evaluate(an,am,grid,alpha,delta):
    ...
    new_cell = cell(grid[an][am],grid[an+1][am],grid[an+1][am+1],grid[an][am+1],grid[an-1][am],
    grid[an][am-1],grid[an-1][am-1],grid[an+1][am-1],grid[an-1][am+1])
